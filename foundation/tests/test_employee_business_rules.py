...
1) Identity (national_code mandatory/normalized/unique)
2) Numbering (employee_no autogenerated, fixed mask)
3) Org role (org_track + designation required)
4) Immutability (national_code & employee_no locked after submit)
"""
import re
import pytest
import frappe
...
def _get_national_code(doc):
    ...
    return (doc.get("national_code") or doc.get("nat_id"))
...
class TestEmployeeIdentity(FrappeTestCase):
    def test_national_code_is_mandatory(self):
        with pytest.raises(Exception):
            make_min_employee(national_code=None)

    def test_national_code_normalizes_to_digits(self):
        e = make_min_employee(national_code="  123-456-7890  ")
        assert _get_national_code(e) == "1234567890"

    def test_national_code_uniqueness(self):
        make_min_employee(national_code="2223334445")
        with pytest.raises(Exception):
            make_min_employee(national_code="2223334445")
...
class TestEmployeeNumbering(FrappeTestCase):
    def test_employee_no_autogenerated_four_digits(self):
        e = make_min_employee(national_code="3334445556", company_code="SMW", province3="TEH")
...
#         e = make_min_employee(national_code="7778889990")
...
#         if hasattr(e, "national_code"):
#             e.national_code = "1112223334"
#         else:
#             e.nat_id = "1112223334"
